...
public int kostka_pierwsza = kostka_druga = suma_na_kostkach = -1;
public boolean koniec_kolejki = budujemy_droge = budujemy_osade = budujemy_miasto = false; // podswietl_wszystkie_mozliwosci -- dodałbym taki przycisk, wydaje się rozsądny
...
public void better_gracz_dzialania(){
        ...
        while(!koniec_kolejki) {
                if (budujemy_droge) {
                        ...
                        /* TODO ZŁAP WSPÓŁRZĘDZNE KTÓRE WYBRAŁ GRACZ */
                        if (zbuduj_miasto_ktora_wybral_gracz(G, x, y)) {
                                if (G.czy_mozna_postawic_osade) {
                                        dostepne_osady = G.znajdz_wszyskie_dostepne_lokalizacje_osad();
                                } else {
                                        dostepne_osady = new ArrayList<Wierzcholek>();
